...
void TM1637Display::DisplayF(float value) {
	uint8_t k;
	uint32_t mul=1000;
	int p= value;	// keep the integer value

	// looking for dot pos, assuming 4 7-segments are presents
  	for(k=0x80; (p>9)&&(k!=1) ;k>>=1) {
  		p/=10;
  		mul/=10;
  	}
  	if(mul==0) mul=1;
                                    ...
                                    uint8_t length, uint8_t pos)
{
	...
	if (num == 0 && !leading_zero) {
		...
		// i will start from left to the right
		int8_t i=length;
		uint8_t digit;
		for(;num!=0;num/=base) {

			if(i==0) {
			 	// shift right all digits (loose the LSB so that the MSB will be aligned on the left)
				ShiftRight(digits,length-1,0);
			}
			else
				--i;
...
void TM1637Display::ShiftRight(uint8_t digits[], uint8_t start, uint8_t end) {
