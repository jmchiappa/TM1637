...
public:
  ...
  //!        the number must be between 0 to 99)
  //! @param pos The position of the most significant digit (0 - leftmost, 3 - rightmost)
  void showNumberHexEx(uint32_t num, uint8_t dots = 0, bool leading_zero = false, uint8_t length = 4, uint8_t pos = 0);
...
protected:
   ...
   void ShiftRight(uint8_t *digits, uint8_t start, uint8_t end);
